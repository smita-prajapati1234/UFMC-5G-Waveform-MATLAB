...
numFFT = [512 1024 2048];        % number of FFT points
...
for sloop=1:3
    subbandOffset = (numFFT(sloop)/2)-(subbandSize*numSubbands/2)      %subbandSize*numSubbands/2 for band center
% Transmit-end processing
%  Initialize arrays
inpData = zeros(bitsPerSubCarrier*subbandSize, numSubbands);
txSig = complex(zeros(numFFT(sloop)+filterLen-1, 1));
snrdB = 1:2:32;              % SNR in dB
...
% Add WGN
rxSig = awgn(txSig, snrdB(loop), 'measured');

%%

yRxPadded = [rxSig; zeros(2*numFFT(sloop)-numel(txSig),1)];
...
prototypeFilterInv = 1./prototypeFilterFreq(numFFT(sloop)/2-subbandSize/2+(1:subbandSize));
...
ber1(loop)=ber(1);
disp(['UFMC Reception, BER = ' num2str(ber1(loop)) ' at SNR = ' ...
