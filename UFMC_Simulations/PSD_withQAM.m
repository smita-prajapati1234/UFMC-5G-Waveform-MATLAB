...
clear all
close all
s = rng(648);       % Set RNG state for repeatability
numFFT = 2048;        % number of FFT points
subbandSize = 20;    % must be > 1 
numSubbands = 10;    % numSubbands*subbandSize <= numFFT
subbandOffset = 924; % numFFT/2-subbandSize*numSubbands/2 for band center
%clr={'b','g','k','r','m','c','--gs','o','y'}
...
ylabel('PSD (dBW/Hz)')
title(['UFMC (256 QAM), ' num2str(numSubbands) ' Subbands, '  ...
