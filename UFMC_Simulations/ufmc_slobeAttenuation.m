...
subbandOffset = 412; % numFFT/2-subbandSize*numSubbands/2 for band center
clr={'b','g','k','r'}
% Dolph-Chebyshev window design parameters
filterLen = 63;      % similar to cyclic prefix length
slobeAtten = [5 10 20 40];     % side-lobe attenuation, dB
%prototypeFilter = chebwin(filterLen, slobeAtten);
for sloop= 1:4
%prototypeFilter = kaiser(filterLen, 2.5);
prototypeFilter = chebwin(filterLen, slobeAtten(sloop));
...
xlabel('SNR')
ylabel('BER')
title('BER vs SNR for UFMC (256QAM, 1024 FFT) at Various Filter Side LObe Attenuation')
end