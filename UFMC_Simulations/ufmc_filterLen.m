...
filterLen = [63 83 103 123];      % similar to cyclic prefix length
...
rxf = [prototypeFilter.*exp(1i*2*pi*0.5*(0:filterLen(sloop)-1)'/numFFT); ...
       zeros(numFFT-filterLen(sloop),1)];
...
xlabel('SNR')
ylabel('BER')
title('BER vs SNR for UFMC (256QAM, 1024 FFT) at Various Filter Length')
end